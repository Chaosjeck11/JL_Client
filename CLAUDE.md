...
- L4+ **Bezahlen** button (green, when status ≠ BEZAHLT, `canPayBeitraege()`): opens `BezahlenBeitragModal` which creates an EINZAHLUNG transaction against the `isMitgliedsbeitrag` category, triggering automatic beitrag tracking. Pre-fills amount with the open balance. Fields: amount, date, **Buchungsjahr** (Kassenbuch year, defaults to the newest `BusinessYear`), tag (ONLINE/BAR). The transaction is sent with `businessYearId: <chosen Buchungsjahr>` and `beitragYearId: <the Beitrag's own businessYearId>` — this decouples which Geschäftsjahr the booking appears in (Kassenbuch) from which Geschäftsjahr's Beitrag gets credited, so e.g. a 2023 Beitrag paid in 2025 shows up in the 2025 Kassenbuch while still marking the 2023 Beitrag as paid. A hint is shown when the two years differ.
